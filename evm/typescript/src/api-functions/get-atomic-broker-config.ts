 ...
 * 
 * @returns {Promise<EvmAtomicBrokerConfig | undefined>} An EvmAtomicBrokerConfig object or undefined 
 *  if an error occurs.
 * 
 * @dev The Atomic Broker configuration lists its address, data on the USDM tokens,
 *  and all collaterals supported by the Broker. 
 */
export async function getAtomicBrokerConfig(isTestnet = false): Promise<EvmAtomicBrokerConfig | undefined> {
    ...
    let url = `${process.env.M1_API_BASE_URL}/ethereum/atomic-broker`;

    if (isTestnet) {
        url += "?isTestnet=true"
    }

    return await getFromAPI<EvmAtomicBrokerConfig>(url, false);
}