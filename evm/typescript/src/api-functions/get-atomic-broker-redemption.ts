import { EvmAtomicBrokerRedemption } from "../interfaces";
 ...
 *  returns an atomic broker redemption object.
 * 
 * @param {string} redeemerAddress The address of a redeemer.
 * @param {boolean} isTestnet Flag to switch between Sepolia and Mainnet.
 * 
 * @returns {Promise<EvmAtomicBrokerRedemption | undefined>} An atomic broker redemption object or undefined
 *  if an error occurs.
 * 
...
export async function getAtomicBrokerRedemption(
    ...
    isTestnet = false): Promise<EvmAtomicBrokerRedemption | undefined> {
    ...
    return await getFromAPI<EvmAtomicBrokerRedemption>(url, true)
}
