...
/**********************************************************************************
 * Typescript function that calls the M1 API Solana endpoint for a collateral
 * price attestation.
 *
 ...
 * @param {string} collateral The collateral token address.
 ...
 */
export async function getSolanaCollateralPriceAttestation(
    requesterAddress: string,
    direction: Exclude<AttestationDirectionRequest, "swap">,
    collateral: string,
    isTestnet = false): Promise<SolanaPriceAttestationResponse | undefined> {

    const url = `${getM1ApiV2BaseUrl()}/price-attestations/collateral/solana`;

    return await postToAPI<SolanaPriceAttestationResponse>(
        url,
        {
